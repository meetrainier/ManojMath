...
ON_Circle::ON_Circle( const MnjPoint& C, double r )
{
  Create( C, r );
}

ON_Circle::ON_Circle( const ON_Plane& pln, const MnjPoint& C, double r )
{
  Create( pln, C, r );
}
...
ON_Circle::ON_Circle( const MnjPoint& P, const MnjPoint& Q, const MnjPoint& R )
{
  Create(P,Q,R);
}
...
const MnjPoint& ON_Circle::Center() const
{
  return plane.origin;
}

const MnjVector& ON_Circle::Normal() const
{
  return plane.zaxis;
}
...
MnjBoundingBox ON_Circle::BoundingBox() const
{
  MnjBoundingBox bbox;
  MnjPoint corners[4]; // = corners of square that contains circle
...
bool ON_Circle::Transform( const ON_Xform& xform )
{
  ...
  else
  {
    ...
    MnjVector V = xform*plane0.xaxis;
    a = V*plane.xaxis;
    b = V*plane.yaxis;
    if (fabs(a) >= fabs(b))
    {
...
bool ON_Circle::Create( const MnjPoint& C, double r )
{
...
{
  return Create(MnjPoint(P),MnjPoint(Q),MnjPoint(R));
}
...
bool ON_Circle::Create(
  const MnjPoint& P,      // [IN] point P
  const MnjVector& Pdir, // [IN] tangent at P
  const MnjPoint& Q      // [IN] point Q
  )
{
  bool rc = false;
  double a, b;
  MnjVector QP, RM, RP, X, Y, Z;
  MnjPoint M, C;
  MnjLine A, B;
...
bool ON_Circle::IsInPlane( const ON_Plane& plane, double tolerance ) const
{
  ...
  int i;
  for ( i = 0; i < 8; i++ ) {
    d = plane.plane_equation.ValueAt( PointAt(0.25*i*M_PI) );
    if ( fabs(d) > tolerance )
      return false;
  }
  return true;
...
MnjPoint ON_Circle::PointAt( double t ) const
{
  return plane.PointAt( cos(t)*radius, sin(t)*radius );
}

MnjVector ON_Circle::DerivativeAt( 
...
MnjVector ON_Circle::TangentAt( double t ) const
{
  MnjVector T = DerivativeAt(1,t);
  T.Unitize();
  return T;
}

bool ON_Circle::ClosestPointTo( const MnjPoint& point, double* t ) const
{
  bool rc = true;
  if ( t ) {
    double u, v;
...
MnjPoint ON_Circle::ClosestPointTo( const MnjPoint& point ) const
{
  MnjPoint P;
  MnjVector V = plane.ClosestPointTo( point ) - Center();
  if ( V.Unitize() ) {
    V.Unitize();
    P = Center() + Radius()*V;
  }
...
int ON_Circle::GetNurbForm( ON_NurbsCurve& nurbscurve ) const
{
  ...
  if ( IsValid() ) {
    ...
    nurbscurve.m_knot[2] = nurbscurve.m_knot[3] = 0.5*M_PI;
    nurbscurve.m_knot[4] = nurbscurve.m_knot[5] = M_PI;
    nurbscurve.m_knot[6] = nurbscurve.m_knot[7] = 1.5*M_PI;
    nurbscurve.m_knot[8] = nurbscurve.m_knot[9] = 2.0*M_PI;
