...
public:
  ...
  */
  ...
  MnjVector DerivativeAt( 
    int,   // derivative (>=0)
    double // evaluation parameter
    ) const;

  MnjVector TangentAt(double) const;
  ...
  bool Rotate(
        double sin_angle,
        double cos_angle,
        const MnjVector& axis_of_rotation
        );
  ...
  bool Rotate(
        ...
        double cos_angle,
        const MnjVector& axis_of_rotation,
        const MnjPoint& center_of_rotation
        );
  ...
  */
  ...
  Parameters:
    circle_radians_parameter - [in] 0.0 to 2.0*M_PI
    nurbs_parameter - [out]
  Example:

          ON_Circle circle = ...;
          double circle_t = 1.2345; // some number in interval (0,2.0*M_PI).
          double nurbs_t;
...
public:
  ...
  //MnjPoint m_point[3]; // 3 points on the circle
};
