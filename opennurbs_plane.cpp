...
const ON_Plane ON_xy_plane( MnjPoint(0.0,0.0,0.0),
                              MnjVector(1.0,0.0,0.0),
                              MnjVector(0.0,1.0,0.0) 
                              );

const ON_Plane ON_yz_plane( MnjPoint(0.0,0.0,0.0),
                              MnjVector(0.0,1.0,0.0), 
                              MnjVector(0.0,0.0,1.0) 
                              );

const ON_Plane ON_zx_plane( MnjPoint(0.0,0.0,0.0),
                              MnjVector(0.0,0.0,1.0),
                              MnjVector(1.0,0.0,0.0)
                              );
...
ON_Plane::ON_Plane(
    const MnjPoint&  P, // point on the plane
...
ON_Plane::ON_Plane(
    const MnjPoint& P,  // point on the plane
    const MnjPoint& Q,  // point on the plane
    const MnjPoint& R   // point on the plane
    )
{
...
MnjPoint ON_Plane::PointAt( double s, double t ) const
{
  return (origin + s*xaxis + t*yaxis);
}

MnjPoint ON_Plane::PointAt( double s, double t, double c ) const
{
  return (origin + s*xaxis + t*yaxis + c*zaxis);
}


MnjLine ON_Plane::IsoLine( // iso parametric line
...
double ON_Plane::DistanceTo( const MnjPoint& point ) const
{
  // signed distance
...
bool ON_Plane::GetDistanceToBoundingBox(const MnjBoundingBox& Box,
				                                double* min, double* max) const
{
  //min and max signed distance.  Returns false if plane normal has zero length.

  MnjVector UnitNormal = Normal();
...
//double ON_Plane::EquationAt( const MnjPoint& p) const
//{
...
const MnjPoint& ON_Plane::Origin() const
{
  return origin;
}

const MnjVector& ON_Plane::Xaxis() const
{
  return xaxis;
}

const MnjVector& ON_Plane::Yaxis() const
{
  return yaxis;
}

const MnjVector& ON_Plane::Normal() const
{
  return zaxis;
}

void ON_Plane::SetOrigin( const MnjPoint& origin_point)
{
  origin = origin_point;
  UpdateEquation();
}
...
bool ON_Plane::ClosestPointTo( MnjPoint p, double* s, double* t ) const
{
...
MnjPoint ON_Plane::ClosestPointTo( MnjPoint p ) const
{
  double s, t;
    ...
    )
{
  ...
  xaxis = X;
  xaxis.Unitize();
  yaxis = Y - DotProduct( Y, xaxis)*xaxis;
  yaxis.Unitize();
...
bool ON_Plane::IsValid() const
{
  ...
  MnjVector N = plane_equation;
  N.Unitize();
  x = FlowDotProduct( N, zaxis );
...
{
  MnjPoint origin_pt = xform*origin;
  // use care tranforming vectors 
  MnjVector xaxis_vec = (xform*(origin+xaxis)) - origin_pt;
  MnjVector yaxis_vec = (xform*(origin+yaxis)) - origin_pt;
      ...
      )
{
  bool rc = true;
  if ( axis == zaxis ) {
    MnjVector x = c*xaxis + s*yaxis;
    MnjVector y = c*yaxis - s*xaxis;
    xaxis = x;
    yaxis = y;
  }
  else {
    MnjPoint origin_pt = origin;
...
bool ON_Plane::Rotate(
      ...
      const MnjVector& axis, // axis of rotation
      const MnjPoint& center // center of rotation
      )
{
  bool rc = false;
  ON_Xform rot;
...
bool ON_Plane::Rotate(
      ...
      const MnjVector& axis,  // axis of rotation
      const MnjPoint& origin_pt  // center of rotation
      )
{
...
int ON_ArePointsOnPlane( // returns 0=no, 1 = yes, 2 = pointset is (to tolerance) a single point on the line
        ...
        const MnjBoundingBox& bbox, // if needed, use ON_GetBoundingBox(dim,is_rat,count,stride,point)
