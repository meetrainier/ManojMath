...
ON_Arc::ON_Arc( const MnjPoint& C, double r, double angle_in_radians )
       : m_angle(0.0,2.0*M_PI)
{
  Create( C, r, angle_in_radians );
}

ON_Arc::ON_Arc( const ON_Plane& pln, const MnjPoint& C, double r, double angle_in_radians )
       : m_angle(0.0,2.0*M_PI)
{
...
ON_Arc::ON_Arc( const MnjPoint& P, const MnjPoint& Q, const MnjPoint& R )
       : m_angle(0.0,2.0*M_PI)
{
  Create( P, Q, R );
}
  ...
  )
{
  ...
  if ( m_angle.Length() > 2.0*M_PI )
  {
    rc = false; // bogus input
    m_angle.m_t[1] = m_angle.m_t[0] + 2.0*M_PI;
  }
  if ( rc )
    rc = IsValid();
  return rc;
...
bool ON_Arc::Create( // arc through 3 3d points
  const MnjPoint& P, // point P
  const MnjPoint& Q, // point Q
  const MnjPoint& R  // point R
  )
{
  ON_Circle c(P,Q,R);
  double a = 0.0;
...
{
  return Create( MnjPoint(P), MnjVector(Pdir), MnjPoint(Q) );
}

//////////
// Create an arc from a 3d start point, 3d start direction, and 3d end point.
bool ON_Arc::Create(
  const MnjPoint& P, // [IN] start point
  const MnjVector& Pdir,  // [IN] arc direction at start
  const MnjPoint&  Q   // [IN] end point
  )
{
  double a=0.0;
  ...
  if ( rc ) {
    ...
    m_angle.m_t[1] = a;
    if (a <= ON_ZERO_TOLERANCE || a >= 2.0*M_PI-ON_ZERO_TOLERANCE )
      rc = false;
  }
  return rc;
}
...
MnjPoint ON_Arc::StartPoint() const
{
  return PointAt(m_angle[0]);
}

MnjPoint ON_Arc::MidPoint() const
{
  return PointAt(m_angle.Mid());
}

MnjPoint ON_Arc::EndPoint() const
{
...
bool ON_Arc::IsValid() const
{
  return (    ON_Circle::IsValid() 
           ...
           && AngleRadians() <= 2.0*M_PI+ON_ZERO_TOLERANCE) 
         ? true : false;
}

MnjBoundingBox ON_Arc::BoundingBox() const
{
       ...
       ) const
{
  if (bGrowBox)
  {
    MnjBoundingBox arc_bbox = BoundingBox();
    bbox.Union(arc_bbox);
  }
  else
    bbox = BoundingBox();
...
{
  return (fabs(fabs(AngleRadians()) - 2.0*M_PI) <= ON_ZERO_TOLERANCE) 
         ? true : false;
}
...
{
  return SetAngleRadians( (a/180.0)*M_PI );
}
...
bool ON_Arc::Trim( ON_Interval domain)
{
  ...
  if(domain[0]<domain[1] && domain[1]-domain[0]<=2.0 * M_PI+ON_ZERO_TOLERANCE){
		m_angle = domain;
    if (m_angle.Length() > 2.0*M_PI) m_angle[1] = m_angle[0] + 2.0*M_PI;
    ok = true;
  }
	return ok;
}
...
int ON_Arc::GetNurbForm( ON_NurbsCurve& nurbscurve ) const
{
  ...
  if ( IsValid() ) {
    if ( IsCircle() )
      ...
      if (angle <= 0.5*M_PI + ON_ZERO_TOLERANCE) {
		    span_count = 1;
        dt = 0.5;
      }
      else if (angle <= M_PI + ON_ZERO_TOLERANCE) {
		    span_count = 2;
  		  angle *= 0.5;
        dt = 0.25;
      }
      else if (angle <= 1.5*M_PI + ON_ZERO_TOLERANCE) {
		    span_count = 3;
  		  angle /= 3.0;
        dt = 1.0/6.0;
      }
       ...
       ) const
{
  ...
  if (rc) {
    if ( *t < m_angle[0] ) {
      a = 0.5*(m_angle[0] + m_angle[1] - 2.0*M_PI);
      if ( *t < a )
        *t = m_angle[1];
      else 
        *t = m_angle[0];
...
MnjPoint ON_Arc::ClosestPointTo( 
       const MnjPoint& pt
       ) const
{
  double t = m_angle[0];
